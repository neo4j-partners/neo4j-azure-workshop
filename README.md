...
[![Open in Dev Containers](https://img.shields.io/static/v1?label=Dev%20Containers&message=Open&color=blue&logo=visualstudiocode)](https://vscode.dev/redirect?url=vscode://ms-vscode-remote.remote-containers/cloneInVolume?url=https://github.com/neo4j-partners/foundry-agents)
...
2. **Resource Group prompt:** Enter your existing resource group name, or leave blank to create one later
...
3. Wait ~3 minutes for container to build
4. Run:
   ```bash
   ...
   # Run setup script (creates environment, lets you pick region)
...
5. **Follow the prompts:**
   - `Select an Azure Subscription:` → Pick your subscription
   - `Pick a resource group:` → Select your existing RG, or "Create a new resource group"

> **Supported Regions:** Azure AI Foundry Agent Service requires `eastus2`, `swedencentral`, or `westus2`
...
5. **Create environment and set region:**
   ```bash
   # Create a new azd environment
   azd env new mydev

   # Set region (REQUIRED - Azure AI Foundry only supports select regions)
   azd env set AZURE_LOCATION eastus2
   ```
   > **Supported Regions:** `eastus2`, `swedencentral`, or `westus2`

6. **Set up resource group (optional):**
   ```bash
...
7. **Provision infrastructure and run:**
   ```bash
   azd up                        # Provision Azure resources
   uv run setup_env.py           # Pull environment variables
   uv run uvicorn api.main:create_app --factory --reload
   ```

8. **Test the API** at `http://localhost:8000`

### Troubleshooting

- **Region not supported error:** Azure AI Foundry Agent Service only works in select regions:
  ```bash
  azd env set AZURE_LOCATION eastus2   # or swedencentral, westus2
  azd up
  ```
- **Container fails to build:** Ensure Docker Desktop is running and has at least 8GB memory allocated
- **Azure login issues:** Run `az login --use-device-code` for browser-based authentication
- **Port 8000 in use:** The container will automatically forward to an available port
- **RoleAssignmentExists error:** If redeploying and you get role assignment errors, skip them:
  ```bash
